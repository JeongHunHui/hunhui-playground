...
module.exports = async function(req, res) {
  ...
  try {
    ...
    const url = `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${encodeURIComponent('작업현황')}!A:H`;
    const d = await (await fetch(url,{headers:{Authorization:`Bearer ${token}`}})).json();
    const rows = d.values || [];
    if (rows.length < 2) return res.status(200).json({status:'idle',updatedAt:null});
    const [header, ...data] = rows;
    const latest = data[data.length - 1];
    const obj = {};
    header.forEach((k,i) => obj[k] = latest[i] || '');
